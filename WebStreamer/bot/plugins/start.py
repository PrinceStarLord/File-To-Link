...
async def start(b, m):
    ...
    if Var.FORCE_UPDATES_CHANNEL:
        try:
            ...
            await b.send_message(
                chat_id=m.chat.id,
                text="<i>Jᴏɪɴ Mʏ Uᴘᴅᴀᴛᴇs Cʜᴀɴɴᴇʟ Tᴏ Usᴇ Mᴇ 🔐</i>",
            ...
            await b.send_message(
                chat_id=m.chat.id,
                text=f"<i>Sᴏᴍᴇᴛʜɪɴɢ Wʀᴏɴɢ Cᴏɴᴛᴀᴄᴛ Mʏ Dᴇᴠᴇʟᴏᴘᴇʀ</i> <b><a href='https://t.me/{Var.UPDATES_CHANNEL}'>[ ᴄʟɪᴄᴋ ʜᴇʀᴇ ]</a></b>",
                parse_mode=ParseMode.HTML,
                disable_web_page_preview=True)
            return
    await m.reply_text(
...
@StreamBot.on_message(filters.command('id') & filters.private)
async def start(b, m):
    await b.send_message(
        chat_id=m.chat.id,
